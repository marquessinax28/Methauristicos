...
values = np.array([rastrigin(x, y) for x, y in sources])

# Para ABC a veces se usa fitness = 1/(1+f) si es minimización; aquí maximizamos rastrigin,
...
def dibujar(iteracion):
    ...
    current_vals = np.array([rastrigin(x,y) for x,y in sources])
...
for t in range(1, NUM_ITERACIONES + 1):
    ...
    for i in range(NUM_FUENTES):
        if trials[i] >= LIMIT:
            ...
            values[i] = rastrigin(sources[i][0], sources[i][1])
            trials[i] = 0
            history[i].append(sources[i].copy())
