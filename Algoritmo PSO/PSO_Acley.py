...
pbest_pos = pos.copy()
pbest_val = np.array([ackley(x,y) for x,y in pbest_pos])
...
def dibujar(iteracion):
    ...
    fitness_curr = np.array([ackley(x,y) for x,y in pos])
    ax2.scatter(pos[:,0], pos[:,1], fitness_curr, c='blue', s=30,) 
    pbest_vals = np.array([ackley(x,y) for x,y in pbest_pos])
...
for t in range(1,NUM_ITERACIONES + 1):
    ...
    fitness = np.array([ackley(x,y) for x,y in pos])
